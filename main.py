...
linewidth = 50
max_lines_per_block = 4 # tbd

# variables for loop inner state
wrapper = textwrap.TextWrapper(width=linewidth, fix_sentence_endings=True, drop_whitespace=True)
length = 0
cap= ''
begin = 0.0
end = 0.0
...
for line in fetched:
    ...
    # 
    if (cap.endswith(('.', ':', '!', '?')) and ( (end - begin > 10.0) or ( len(cap) > (linewidth * max_lines_per_block) ) )):
